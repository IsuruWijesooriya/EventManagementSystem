...
const ManageEvent = () => {
  ...
  const handleEditClick = (event) => {
    ...
    setImagePreview(`https://34.121.130.254:3000/${event.image}`);
    setIsModalOpen(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`https://34.121.130.254:3000/api/events/${currentEvent.id}`, {
        method: 'PUT',
        body: formData,
      });
  ...
  const deleteEvent = async (eventId) => {
    try {
      const response = await fetch(`https://34.121.130.254:3000/api/events/${eventId}`, {
        method: 'DELETE',
      });
