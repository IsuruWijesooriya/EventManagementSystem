...
const ManageEvent = () => {
  ...
  const handleEditClick = (event) => {
    ...
    setImagePreview(`https://shopylk.com/${event.image}`);
    setIsModalOpen(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`https://shopylk.com/api/events/${currentEvent.id}`, {
        method: 'PUT',
        body: formData,
      });
  ...
  const deleteEvent = async (eventId) => {
    try {
      const response = await fetch(`https://shopylk.com/api/events/${eventId}`, {
        method: 'DELETE',
      });
