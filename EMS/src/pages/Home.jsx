...
const Home = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        const response = await fetch('https://shopylk.com/api/events'); // Adjust URL based on your setup
  ...
  <div className="featured-card" onClick={() => openModal(latestEvents[currentFeatured])}>
    <img
      src={`https://shopylk.com/${latestEvents[currentFeatured]?.image}?t=${Date.now()}`}
              ...
              {currentEvents.map((event) => (
                <div key={event.id} className="event-card1" onClick={() => openModal(event)}>
                  <div className="event-card1-image">
                    <img src={`https://shopylk.com/${event.image}`} alt={event.name} />
                  </div>
