...
const Home = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        const response = await fetch('https://34.136.230.234:3000//api/events'); // Adjust URL based on your setup
  ...
  <div className="featured-card" onClick={() => openModal(latestEvents[currentFeatured])}>
    <img
      src={`https://34.136.230.234:3000//${latestEvents[currentFeatured]?.image}?t=${Date.now()}`}
              ...
              {currentEvents.map((event) => (
                <div key={event.id} className="event-card1" onClick={() => openModal(event)}>
                  <div className="event-card1-image">
                    <img src={`https://34.136.230.234:3000//${event.image}`} alt={event.name} />
                  </div>
